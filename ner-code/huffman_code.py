...
class HuffmanTree(object):
    ...
    def __init__(self,char_weights,save_path=""):
        ...
        while len(self.a)!=1:
            self.a.sort(key=lambda node:node._value,reverse=False)
            # 每次选取差值较小的组成一个数组，再比较差值相等情况下数组排序
            arr = []
            for i in range(len(self.a)):
                for j in range(i+1,len(self.a)):
                    arr.append((self.a[i],self.a[j]))
            arr.sort(key=lambda line:abs(line[0]._value-line[1]._value))
            # for i in range(len(arr)):
            #     print(arr[i][0]._value,arr[i][1]._value)
...
if __name__=='__main__':
    ...
    print(char_weights)
    tree = HuffmanTree(char_weights, '/Users/zc/ztt/ner_code/data/cluner/cluener_txt/all_Huffman_labels.csv')
    tree.get_code()
